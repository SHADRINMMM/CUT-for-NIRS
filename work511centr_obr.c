...
const int nbins = 6;	
  ...
  //для сигнала
  TH1F *Hist_pt_S = new TH1F("Pt_balance", "Entries&Pt_balance;Pt-balabnce ;Entries", 50, 0, 1.0);
  TH1F *Hist_centr_S = new TH1F("zet", "centrality;centrality;Entries", 50, 0, 1);
  TH1F *Hist_mass_S = new TH1F("Pt_balance", "Entries&Pt_balance;Pt-balabnce ;Entries", 50, 0, 1.0);
  TH1F *Hist_rap_S = new TH1F("zet", "centrality;P_{T} [GeV];Entries", 50, 0, 10);
  
  //для форна
  TH1F *Hist_pt_B = new TH1F("Pt_balance", "Entries&Pt_balance;Pt-balabnce;Entries", 50, 0,1.0);
  TH1F *Hist_centr_B = new TH1F("zet", "centrality;centrality;Entries", 50, 0, 1);
  TH1F *Hist_mass_B = new TH1F("Pt_balance", "Entries&Pt_balance;Pt-balabnce;Entries", 50, 0,1.0);
  TH1F *Hist_rap_B = new TH1F("zet", "centrality;P_{T} [GeV];Entries", 50, 0, 10);
  
  //для значимостей
  TH1F *Hist_zn_pt= new TH1F("importance of kriteria of pt-balabnce", "importance&level;level[GeV]; importance;",50, 0,1.0);
  TH1F *Hist_zn_centr= new TH1F("importance of kriteria of centrality", "importance&level;level[GeV]; importance;", 21, 0,1.0);
  TH1F *Hist_zn_mass= new TH1F("importance of kriteria of mass", "importance&level;level[GeV]; importance;", 50, 0,1.0);
  TH1F *Hist_zn_rap= new TH1F("importance of kriteria of rap", "importance&level;level[GeV]; importance;", 50, 0,1.0);
...
// с индексом _S - собыите  
 ...
 // cчитывание данных из дерева 
   ...
   Double_t  level_rap=0;
   Double_t  level_centr=1.0;
  ...
  //считываение количества вхождений в дерево
   ...
   //while (level_pt<=1){
	...
	/*
			Kr_S=Kr_S+1;
			//cutflow_S->Fill(1);
			if((lead_S.M()+sub_lead_S.M())>level_mass) Kr_mass_S=Kr_mass_S+1;
			//cutflow_S->Fill(2);
	*/
			//кртерий на разность быстрот
	/*
			if((abs(lead_S.Rapidity()-sub_lead_S.Rapidity()))>level_rap) Kr_rapidity_S=Kr_rapidity_S+1;
			//cutflow_S->Fill(3);
...
//			if (zet_S>1000) cout<<"1"<<endl;
	...
	/*
			...
			Hist_pt_S->Fill(PT_BALANCE_S);
			//if(PT_BALANCE_S>level_pt)
			if(PT_BALANCE_S<level_pt) Kr_pt_S=Kr_pt_S+1;
			//cutflow_S->Fill(5);
	*/
  ...
  for ( Long64_t j = 0; j < nEvents_B; j++)
	...
	*/
	//кртерий на разность быстрот
	/*
			if((abs(lead_B.Rapidity()-sub_lead_B.Rapidity()))>level_rap) Kr_rapidity_B=Kr_rapidity_B+1;
			//cutflow_B->Fill(3);
	*/
	//критерий на центральность 
	...
	/*
			...
			Hist_pt_B->Fill(PT_BALANCE_B);
			//if(PT_BALANCE_B>level_pt)
			 if(PT_BALANCE_B<level_pt)	Kr_pt_B=Kr_pt_B+1;
			//cutflow_B->Fill(5);
