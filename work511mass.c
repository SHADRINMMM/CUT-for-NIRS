...
const double nmax=300.0;
  ...
  TH1F *Hist_mass_S = new TH1F("mass", "Entries&mass;mass;Entries", 300, 0, 300);
  ...
  TH1F *Hist_mass_B = new TH1F("mass", "mass;mass;Entries", 300, 0,300);
  ...
  TH1F *Hist_zn_mass= new TH1F("importance of kriteria of mass", "importance&level;level[GeV]; importance;", nmax+1, 0,nmax);
  ...
  //считываение количества вхождений в дерево
   ...
   tree_norm->GetEntry(0);
	...
	// заполнение 4 векторов  
			...
			Hist_mass_B->Fill(lead_B.M()+sub_lead_B.M(),weight_real_B);
	  }
	...
	// заполнение 4 векторов 
			...
			Hist_mass_S->Fill(lead_S.M()+sub_lead_S.M(),weight_real_S);
		}
   // pt-balance
//	while (level_pt<=1){
	// centrality
//	while (level_centr<=1){   
	...
	/*
			...
			Hist_pt_S->Fill(((abs(lead_S.Rapidity()-sub_lead_S.Rapidity())))*weight_real_S);
...
//			if (zet_S>1000) cout<<"1"<<endl;
  ...
  for ( Long64_t j = 0; j < nEvents_B; j++)
	...
	//критерий на инвариантную массу 
			...
			Kr_B=Kr_B+1;
			if((lead_B.M()+sub_lead_B.M())>level_mass) Kr_mass_B=Kr_mass_B+weight_real_B;
			cutflow_B->Fill(2);
			Hist_mass_B->Fill(lead_B.M()+sub_lead_B.M(),weight_real_B);
...
//	cout<<max_Zn_Kr_cetr<<"\n"<<Zn_Kr_cetr<<"\n"<<"\n";
	...
	cout<<"max_Zn_Kr_pt="<< max_Zn_Kr_pt<<endl;
	cout<<"level_pt"<< max_level_pt<<endl ;
